...
CREATE TABLE `goods` (
  ...
  `img_filename` varchar(100) NOT NULL DEFAULT '',
  `description` varchar(100) NOT NULL DEFAULT '',
  `goods_price` real(4) NOT NULL DEFAULT 0.0,
  `deal_time` timestamp NOT NULL DEFAULT 0,#交易达成的时间
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `status` tinyint(1) NOT NULL DEFAULT 0, #0 正常，未被购买；1，正常，已被购买；2，被购买锁定，但未付款成功；3,状态未定义；9，已删除。
  PRIMARY KEY (`id`),
...
CREATE TABLE  `account`(
  ...
  `balance` real(4) NOT NULL DEFAULT 0.0,
  `mtime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `topuporder` (
  ...
  `addup_amount` real(4) NOT NULL DEFAULT 0.0,
  `mtime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `status` tinyint(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  KEY `stat` (`status`),
