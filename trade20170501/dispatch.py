...
from webframe import App, Jinja2Template, HttpError,ctx,JSONRes
...
def user_login():
	...
	logging.warning('account: ' + account + ' passwd: ' + passwd)

	user = User.find_first('where account=?',account)
	#TODO: add authorize method for user login
	if not user:
...
def logout():
	...
	logging.warning('log_out func is called!!!')
...
def user_register():
	...
	logging.warning('account: ' + account + ' name: ' + name + \
		' password: ' + password +' birthday: ' + birthday + ' phone: ' + phone + ' address: ' + address + ' post: '+ post)
...
def add_goods():
	...
	goods_added.update()
	return tem_redirect('/') #need to modified
...
@app.route(r'/profile$','GET')
def show_user_profile():
	user = ctx.request.user
	user_account = user.account
	logging.warning('user_account: ' + user_account)
	account = Account.find_first('where account = ? and status = 0',user_account)
...
@app.route(r'/api/test$','GET')
def test():
	user = ctx.request.user
	#mydic = db.Dict()
	#mydic.relation = {'myname':'noodle','gfn':'Elisa'}
	#mydic.gf = 'little gangan'
	mydic = {'relation':{'myname':'noodle','gfn':'Elisa'},'gf':'little gangan' }
	# mydic = {'myname':'noodle','gfn':'Elisa'}
	return JSONRes(mydic)
